...
(async () => {
  ...
  const filterFullCliSupport = ([key, type]) => key.endsWith('Settings') || (type.channels.metadataApi.exposed && type.channels.sourceTracking.exposed && inRegistry(key))
  const filterCliNoTracking = ([key, type]) => !key.endsWith('Settings') && type.channels.metadataApi.exposed && !type.channels.sourceTracking.exposed && inRegistry(key)
  const filterCliNoSupport = ([key, type]) => !key.endsWith('Settings') && type.channels.metadataApi.exposed && type.channels.sourceTracking.exposed && !inRegistry(key)

  const [currentOutput, nextOutput] = [currentCoverage, nextCoverage].map((coverage) => ({
    noMetadataAPI: Object.entries(coverage.types).filter(filterNoMetadata).map(([key, type]) => key),
    fullCliSupport: Object.entries(coverage.types).filter(filterFullCliSupport).map(([key, type]) => key),
    cliNoTracking: Object.entries(coverage.types).filter(filterCliNoTracking).map(([key, type]) => key),
    noCliSupport: Object.entries(coverage.types).filter(filterCliNoSupport).map(([key, type]) => key),
  }));

  const additionalCLISupport = Object.values(registryTypes).filter(t => !Object.keys(currentCoverage.types).includes(t.name)).filter(t => !Object.keys(nextCoverage.types).includes(t.name)).map(t => t.name)
  ...
  const getCoverageRows = (coverageTypes: CoverageObject['types'], onlyNew = false) => {
    return Object.keys(coverageTypes).sort().filter( key => onlyNew ? !currentCoverage.types[key]: true).map(typeName => {
...
Currently, there are ${currentOutput.fullCliSupport.length + currentOutput.cliNoTracking.length}/${Object.keys(currentCoverage.types).length} supported metadata types in Salesforce CLI.
...
`;
  ...
  if (shell.exec(`git commit -am "chore: adding types for SDR"`).code !== 0) {
