...
1. Select the type of version bump. Typically using the default value of `patch` is fine. Select `minor` when new backwards compatible functionality is added. Select `major` when incompatible API changes are made. See https://semver.org/ for more information.
