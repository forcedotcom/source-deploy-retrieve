...
export const resolveApiVersion = async (options: ApiVersionConfig): Promise<string> => {
  ...
  if (isString(apiVersionConfig)) {
    getLogger().debug(`Resolved API version to ${apiVersionConfig} from config.`);
    return apiVersionConfig;
  }

  // Use max api version of a specific org
  ...
  if (connection) {
    const maxApiVersion = await connection.retrieveMaxApiVersion();
    getLogger().debug(`Resolved API version to ${maxApiVersion} from org connection.`);
...
export const resolveSourceApiVersion = async (options: SourceApiVersionConfig): Promise<string | undefined> => {
  ...
  if (isString(projectConfig?.sourceApiVersion)) {
    getLogger().debug(`Resolved sourceApiVersion to ${projectConfig.sourceApiVersion} from sfdx-project.json.`);
