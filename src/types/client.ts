...
import { ApiDeployResult } from './newClient';
...
export interface DeployRetrieveClient {
   ...
   */
  deploy(components: SourceComponent | SourceComponent[]): Promise<ApiDeployResult | DeployResult>;
  /**
   * Infer metadata components from source path, deploy them, and wait for results.
   *
   * @param filePath Paths to source files to deploy
   */
  deployWithPaths(paths: SourcePath | SourcePath[]): Promise<ApiDeployResult | DeployResult>;
}
...
export abstract class BaseApi implements DeployRetrieveClient {
  ...
  ): Promise<ApiDeployResult | DeployResult>;
