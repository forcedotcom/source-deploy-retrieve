 ...
 */
export const getReplacementMarkingStream = async (path: string): Promise<ReplacementMarkingStream | undefined> => {
  // remove any that don't agree with current env
  const filteredReplacements = envFilter(await readReplacementsFromProject(path));
 ...
 */
const readReplacementsFromProject = async (path: string): Promise<ReplacementConfig[]> => {
  const proj = await SfProject.resolve(path);
